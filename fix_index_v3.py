...
try:
    ...
    try:
        ...
        print(f"cp1252 failed")
        # Last resort: binary read and decode with ignore
        with open(file_path, 'rb') as f:
            content = f.read().decode('utf-8', errors='replace')
        print("Read as binary with replacement")

# Replacements (Unicode chars are safe in logic, just not in print on some Windows consoles)
replacements = [
    ('??/span>', '</span>'),
    ('??/SPAN>', '</span>'),
    ('??/p>', '</p>'),
    ...
    ('VIEW PUBLICATION ??/span>', 'VIEW PUBLICATION </span>'),
]
...
for old, new in replacements:
    if old in content:
        ...
        count += 1
        # Avoid printing unicode to console
        print(f"Replaced a pattern.")
